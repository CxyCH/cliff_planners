...
struct Data {
  ...
  double distance;
  double upstream;
  Data(double pt_, double cost_, double distance_, double upstream_,
       long int nodes_) {
    upstream = upstream_;
    pt = pt_;
    distance = distance_;
  ...
  for (boost::filesystem::directory_iterator itr(p); itr != end_itr; ++itr) {
    ...
    try {
      ...
      for (const auto m : view) {
        ...
        if (pm != NULL) {
          data.push_back(Data(pm->data[0], pm->data[1], pm->data[2],
                              pm->data[3], (long int)pm->data[4]));
          file_pm << data.back().pt << "," << data.back().cost << ","
                  << data.back().upstream << "," << data.back().distance << ","
                  << data.back().nodes << ";\n";
        }
      }
      ...
      if (data.size() > 0) {
        ...
        file_pm_first << data.front().pt << "," << data.front().cost << ","
                      << data.front().upstream << "," << data.front().distance
                      << "," << data.front().nodes << ";\n";
      }
      ...
      file_paths_first << "\n";
      file_paths_last << "\n";
    } catch (rosbag::BagFormatException &ex) {
      std::cout << "Exception: " << ex.what() << " Skipping file" << std::endl;
    };
